...
size_t pthreadpool_get_max_threads_count(struct pthreadpool* threadpool) {
	if (threadpool == NULL) {
		return 1;
	}

	return threadpool->max_threads_count.value;
}
...
size_t pthreadpool_get_threads_count(struct pthreadpool* threadpool) {
	...
	return pthreadpool_load_relaxed_size_t(&threadpool->threads_count);
}
	...
	uint32_t flags)
{
	...
	if (threadpool != NULL) {
		threads_count = pthreadpool_load_relaxed_size_t(&threadpool->threads_count);
	}
